...
using TouchPoint = struct tp
{
    uint16_t x,       y,          // screen coordinates
        xValue,  yValue, zValue;  // raw values read from touchpad
};

using TouchCalibration = struct tc
{
    int      xMinScreen, yMinScreen, // a point at or near upper left corner
             xMaxScreen, yMaxScreen; // a point at or near lower right corner
    uint16_t xMinValue,  yMinValue,  // corresponding values to 1st point
             xMaxValue,  yMaxValue;  // corresponding values to 2nd point
};
...
class XPT2046_Bitbang 
{
    public:
        ...
        void setCalibration(TouchCalibration &tc);
